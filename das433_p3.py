...
# Here we are determining the homography between the 2D co-ords of a face and
# its world co-ordinate with the origin at one corner, like in the video.  So
# target is in 3D co-ordinates, but with this corner origin.  When computing the
# homography, we don't need Z (depth), it was only used to determine these world
# plane co-ordinates.  We then do an inverse warp from the original 2D image
# plane to this 3D plane. So we get the homography's inverse with np.linalg.inv,
# apply that to each point in the warp target to get a point in the original
# image, and bilinearly interpolate.
...
def computehomography(facefrom, faceto):
    ...
    A = np.array([[x0, y0, 1, 0, 0, 0, -x0p*x0, -x0p*y0, -x0p],
                  [0, 0, 0, x0, y0, 1, -y0p*x0, -y0p*y0, -y0p],
                  [x1, y1, 1, 0, 0, 0, -x1p*x1, -x1p*y1, -x1p],
                  [0, 0, 0, x1, y1, 1, -y1p*x0, -y1p*y1, -y1p],
